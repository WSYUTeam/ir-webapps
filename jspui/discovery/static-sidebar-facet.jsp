...
<%
	...
	for (DiscoverySearchFilterFacet facetConf : facetsConf)
	{
    	...
    	List<FacetResult> facet = mapFacetes.get(f);
 	    ...
 	    if(i!=0 && i%discovery_facet_list == 0) {
 	    %>
 	    </div><div id="facets" class="facetsBox row panel">	
 	    <%
 	    }
 	    i++;
		    ...
		    { 
		        ...
		        %><li class="list-group-item"><span class="badge"><%= fvalue.getCount() %></span> <a href="<%= request.getContextPath()
	                ...
	                <%
	                	...
	                	String type_value = "jsp.search.facet.language." + fvalue.getDisplayedValue().toLowerCase();
		                %>
		                	<fmt:message key="<%= type_value %>" />
		                <%
		        	} else {
