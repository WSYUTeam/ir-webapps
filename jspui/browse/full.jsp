...
<%
	...
	if (bix.isMetadataIndex())
		typeKey = I18nUtil.getMessage("browse.type.metadata." + bix.getName(), UIUtil.obtainContext(request));
	else if (bi.getSortOption() != null)
		typeKey = I18nUtil.getMessage("browse.type.item." + bi.getSortOption().getName(), UIUtil.obtainContext(request));
	else
		typeKey = I18nUtil.getMessage("browse.type.item." + bix.getSortOption().getName(), UIUtil.obtainContext(request));
...
%>
        ...
        <input type="text" class="form-control-fixed" name="starts_with" size="4" maxlength="4"/>
<%
	}
...
%>
		...
		<input type="submit" class="btn btn-default btn-filter-add" name="submit_browse" value="<fmt:message key="jsp.general.update"/>"/>

<%
    if (admin_button && !withdrawn && !privateitems)
    {
        %><input type="submit" class="btn btn-default btn-filter-add" name="submit_export_metadata" value="<fmt:message key="jsp.general.metadataexport.button"/>" /><%
    }
%>
